...
class probnet(nn.Module):
    def __init__(self, image_size = 60, pretrained_vae = None, device = "cpu"):
        ...
        self.double_9 = doubleconv(channels[3]+channels[2], channels[2], channels[2])
    ...
    def sample(self, x, n_samples):
        ...
        for i in range(n_samples):
            ...
            with torch.no_grad():
                x_vae = self.vae(x)
                x_hat = self.forward(x_vae)
                if samples != None:
                    samples = torch.cat([samples, x_hat],dim= 0)
                else:
                    samples = x_hat
...
def test_output(net, noisy,labels, output_folder):
    ...
    for i in [6,30,36,50]:
        ...
        test = net.sample((noisy[i:i+1,:,:,:].to(device)),1)[0]
...
def run(param_file, data_folder, output_folder,predict = False, debug = True):
